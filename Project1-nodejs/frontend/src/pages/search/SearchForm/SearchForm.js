...
function SearchForm({ getData }) {
  ...
  const handleSeacrMovie = async (e) => {
    e.preventDefault();
    ...
    let payload = {
      ...
      genre: genre_value? genre_value : "Action",
      typeMedia: typeMedia_value? typeMedia_value : "Movie",
      language: language_value? language_value : "En",
      year:year_value
    }
    ...
    console.log(payload);
     ...
     try {
      ...
      let res = await fetch("http://localhost:5000/api/movies/search", {
        method: "POST",
        headers: {
      ...
      // if(!res.status) throw Error("Call server unsuccess");

      let {status, movies} = await res.json();
      if(status) {
